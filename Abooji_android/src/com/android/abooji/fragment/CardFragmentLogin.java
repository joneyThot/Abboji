...
public class CardFragmentLogin extends BaseFragment implements OnClickListener
{
	Fragment fr;
	...
	String card,card_type,category_pref;
	int card_pos,card_type_pos;
	...
	private void initview() 
	{
		...
		mRootView.findViewById(R.id.activity_cards_prev_btn).setVisibility(View.GONE);
		mRootView.findViewById(R.id.activity_cards_next_btn).setVisibility(View.GONE);
		mRootView.findViewById(R.id.activity_card_save_btn).setOnClickListener(this);
		...
		card = sharedPreferences.getString(Utils.CARDS, "");
		card_type = sharedPreferences.getString(Utils.CARD_TYPE, "");
		category_pref = sharedPreferences.getString(Utils.CARD_CAT , "");

		card_pos = sharedPreferences.getInt(Utils.CARDS_POS, 0);
		card_type_pos = sharedPreferences.getInt(Utils.CARDS_TYPE_POS, 0);

		mCardSpinner.setSelection(card_pos);
		mCardTypeSpinner.setSelection(card_type_pos);
		mCategoryPref.setText(category_pref);
	...
	public void onClick(View v) 
	{
		...
		case R.id.activity_card_save_btn:
	...
	private void openCategoryDialog() {
		...
		mCardCategoryList = (ListView) dialog.findViewById(R.id.categoryList);		
	...
	private void saveUserData() {
		...
		if(Utils.checkInternetConnection(getActivity())){
			...
			saveUserRequestTask.execute(sharedPreferences.getString(Utils.USER_ID, ""),
					...
					((CardTypeModel)mCardTypeSpinner.getSelectedItem()).card_type_title,
					"",
					((CardCategoryModel)mCardSpinner.getSelectedItem()).card_category_title,
					mCategoryPref.getText().toString(),
				...
				public void onResponseReceived(Object response) {
					...
					if(saveUserModel != null)
					{
						if(saveUserModel.status.equalsIgnoreCase("1")) {
							...
							Logger.e("Card Fragment Get ", "Card :: " + saveUserModel.cards + " Card Type :: " + saveUserModel.card_type +	" Card Category :: " + saveUserModel.card_cat);

							Utils.storeString(sharedPreferences, Utils.CARDS, saveUserModel.cards);
							Utils.storeInt(sharedPreferences, Utils.CARDS_POS, mCardSpinner.getSelectedItemPosition());

							Utils.storeString(sharedPreferences, Utils.CARD_TYPE, saveUserModel.card_type);
							Utils.storeInt(sharedPreferences, Utils.CARDS_TYPE_POS, mCardTypeSpinner.getSelectedItemPosition());

							Utils.storeString(sharedPreferences, Utils.CARD_CAT, saveUserModel.card_cat);

							Logger.e("Card Fragment Save", "Card :: " + mCardSpinner.getSelectedItem().toString() + " Card Type :: " + mCardTypeSpinner.getSelectedItem().toString() + " Card Category :: " + mCategoryPref.getText().toString());
