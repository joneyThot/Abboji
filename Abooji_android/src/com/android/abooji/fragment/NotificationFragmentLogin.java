...
public class NotificationFragmentLogin extends BaseFragment implements OnClickListener 
{
	Fragment fr;
	...
	private void initview() 
	{
		...
		mRootView.findViewById(R.id.activity_notification_alert_prev_btn).setVisibility(View.GONE);
		...
		alert_pos = mSharedPreferences.getInt(Utils.ALERT_POS, 0);
		alert_time = mSharedPreferences.getString(Utils.ALERT_TIME, "");

		mAlert.setSelection(alert_pos);
		mAlerTiming.setText(alert_time);
	}
	@Override
	...
	private void saveUserData() {
		...
		if(Utils.checkInternetConnection(getActivity())){
				...
				public void onResponseReceived(Object response) {
					...
					if(saveUserModel != null)
					{
						if(saveUserModel.status.equalsIgnoreCase("1")) {
							...
							getUserData(mSharedPreferences.getString(Utils.USER_ID, ""));
			...
			saveUserRequestTask.execute(mSharedPreferences.getString(Utils.USER_ID, ""),
					...
					mAlert.getSelectedItem().toString(),
					mAlerTiming.getText().toString());
		}else{
				...
				public void onResponseReceived(Object response) {
					...
					if(saveUserModel != null)
					{
						if(saveUserModel.status.equalsIgnoreCase("1")) {
							...
							Logger.e("Notification Fragment Get ", "Alert :: " + saveUserModel.notification_status + 
									" Alert Time :: " + saveUserModel.notification_alerts);
							
							Utils.storeString(mSharedPreferences, Utils.ALERT, saveUserModel.notification_status);
							Utils.storeInt(mSharedPreferences, Utils.ALERT_POS, mAlert.getSelectedItemPosition());
							
							Utils.storeString(mSharedPreferences, Utils.ALERT_TIME,saveUserModel.notification_alerts );
							
							Logger.e("Notification Fragment Save", "Alert :: " + mAlert.getSelectedItem().toString() + " Alert Time :: " + mAlerTiming.getText().toString());
